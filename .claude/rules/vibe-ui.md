...
- **vibe 完 commit 前必跑 gate**：走 `/vibe-check`（白名單內定向、白名單外 dev 全量；**定向綠只代表選集，`--full` 綠才是全量已驗**；pre-push 只跑煙霧，CI 跑 production 全量）。裸指令 `npx playwright test --config playwright.gate.config.ts` 等價**但少了前置檢查**——gate 範圍尚無測試檔時它回 exit 1（`No tests found`），`/vibe-check` 與 pre-push 則會判定「無 spec 可守」放行
