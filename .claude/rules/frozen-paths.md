...
| 2026-09-24（issue #154，下游同步時 /code-review 發現） | PR #141 收尾把直譯器偵測收斂到指令位置後，`timeout 10 rm`、`nice rm`、`exec rm`、`! rm`、`stdbuf -o0 tee` 加凍結檔全部漏放：前綴不在 `COMMAND_PREFIX`，其後的動詞判不成指令位置。這是 #141 帶進來的改壞舊功能，且 `timeout` 是 Claude 自己跑指令常加的前綴，不屬於「刻意繞道」 | 已修補：`timeout`／`nice`／`exec`／`stdbuf` 補進 `COMMAND_PREFIX`；`!` 由 `isCommandPosition()` 遞迴判斷——`!` 本身在指令位置（段首、或前一個詞是 `COMMAND_PREFIX` 成員）時，其後才算指令位置（放進 `COMMAND_PREFIX` 會讓 `find … ! -name tee` 的 `tee` 被跳旗標邏輯當成子指令而誤擋；只認段首又漏放 `time ! rm`、`then ! rm`）；只對單一 wrapper 吃值的旗標（`nice -n`、`timeout -s`、`stdbuf -o`、`exec -a`）另列 `WRAPPER_OWN_VALUE_FLAGS`，不併進共用表（`sudo -n` 不吃值，併進去會漏放 `sudo -n rm`）；`timeout` 的時限位置參數由 `WRAPPER_POSITIONALS` 跳過。其他 wrapper 列為已知極限第 24 條 |
