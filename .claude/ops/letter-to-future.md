...
**1. 全域層（`~/.claude/`）不在本制度管轄內。**
全域 CLAUDE.md 引用的規則檔若失效，本專案的路由驗證抓不到——發現時提醒使用者處理，不要自己改 `~/.claude/`。
...
3. **rules 檔的 paths 觸發只在主對話生效，subagent 內不注入**——要在 subagent 內生效的規則，skill 必須明文指讀；必須技術強制的（如凍結區）靠 PreToolUse hook。
