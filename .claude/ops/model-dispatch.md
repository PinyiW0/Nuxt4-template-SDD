...
- **升級那次計入 2 輪**：第 1 輪失敗後就決定第 2 輪派誰（同型錯 → 帶失敗軌跡升 opus；錯法不同 → 修 prompt 重派 sonnet），第 2 輪就是最後一輪。`haiku → sonnet` 那一跳不計（理由同上方括號：便宜，不值得花輪次判型）
...
- **使用者裁決過的決策，要留言記到 issue**——換 session 接手的人才查得到「為什麼」。觸發（二擇一，客觀可判）：命中 [judgment-rubrics.md](judgment-rubrics.md) 第 3 節必停清單並經使用者裁決；或偏離 skill 範本的取捨
  - 由主線（不是分身）執行 `gh issue comment <N>` 一則，固定三行「決策：／理由：／捨棄的替代：」，第一行以「決策：」開頭當標記（`/ship` Phase 0 靠這個前綴過濾）；同一輪多筆決策可合併成一則留言，各筆維持三行、空行隔開。`<N>` 限 issue 編號，不對 PR 留言
