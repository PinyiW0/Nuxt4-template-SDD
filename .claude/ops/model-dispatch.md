...
- **升級那次計入 2 輪**：第 1 輪失敗後就決定第 2 輪派誰（同型錯 → 帶失敗軌跡升 opus；錯法不同 → 修 prompt 重派 sonnet），第 2 輪就是最後一輪。「同型」判的是這個錯法重跑會不會再犯：能力盲點（框架語意、規範沒讀懂）→ 升；一次性失誤或 prompt 沒講清楚 → 修 prompt。不必等第二次失敗才判。`haiku → sonnet` 那一跳不計（理由同上方括號：便宜，不值得花輪次判型）
