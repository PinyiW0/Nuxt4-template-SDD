...
直播播放元件的核心（省略純樣式 / 全螢幕等與 HLS 無關的部分）。`hls?: boolean` 控制是否走 HLS 掛載，`src` 為 `.m3u8` URL（或 `null` 表無訊號）。
...
// Safari 原生 HLS 路徑旗標：true 時 template 直接綁 src，不經 hls.js
const canNativeHls = ref(false)
// 致命且無法恢復時為 true → 顯示「無法載入直播」
const hlsError = ref(false)

// hls.js 實例型別只取用到的成員（playingDate 供 PDT 對齊讀「當前畫面的真實時刻」）
let hlsInstance: {
  destroy: () => void
  startLoad: () => void
...
async function setupHls() {
  ...
  if (!props.hls || !props.src || videoRef.value !== video)
    return
  if (!Hls.isSupported()) {
    hlsError.value = true
    return
...
// src / hls 開關變動 → 重新掛載（client only）
watch(() => [props.hls, props.src], () => {
...
onMounted(() => {
  void setupHls()
  onBeforeUnmount(teardownHls) // 完整 teardown：destroy 實例 + 清 timer
})
</script>
...
- `setupHls` 開頭一律先 `teardownHls()`，這樣 src 變動的重掛、與離場 teardown 共用同一條清理路徑。
...
| 類別 | 範例 | 處置 |
|------|------|------|
| fatal NETWORK | manifest / segment 載入致命失敗 | `startLoad()` 重新載流（多數可自行恢復） |
| fatal MEDIA | 解碼器掛掉 | `recoverMediaError()` 恢復媒體管線 |
...
最難纏的不是有 error 的卡頓，而是**沒有任何 error、`currentTime` 卻不再前進**（loop 靜默死掉）。錯誤事件攔不到它，只能靠定時器主動偵測：

- 每 2 秒檢查 `currentTime` 是否比上次前進（>0.1s）。
- 有前進 → 更新基準時間。
- 連續卡超過 6 秒 → `seek` 回 live edge + `startLoad()` 重啟，並把 `lastProgressAt` 往後推（給恢復時間，避免每輪狂觸發）。

這是「不必手動重整就能自己活過來」的關鍵防線。閾值（2s / 6s）依串流特性調。
...
defineExpose({ getPlayingDate, setSyncRate })
```
...
| unmount 沒 `destroy` + 清 timer | 記憶體 / 看門狗 timer 洩漏 | `teardownHls()`：`destroy` + `clearInterval` |
...
- [ ] `teardownHls()`：`destroy` 實例 + 清看門狗 timer；PDT sync 離場還原 rate
