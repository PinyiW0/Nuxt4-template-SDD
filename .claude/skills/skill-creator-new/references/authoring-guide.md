...
跨工具的 Agent Skills 標準只認 `name`、`description`、`license`、`compatibility`、`metadata`、`allowed-tools` 六個欄位。上表其餘欄位都是 Claude Code 專屬擴充——只在 Claude Code 生效，打包上傳到 claude.ai／Skills API 會因未知欄位直接報錯。skill 只在本專案用就不必顧慮；要對外分享才需要收斂到那六個。

硬限制（標準層會驗，超過即失敗）：`name` ≤64 字元、小寫字母數字連字號、不可含 `anthropic` 或 `claude`；`description` ≤1024 字元、不可空、須用第三人稱寫（「處理 Excel 檔案……」而非「我可以幫你……」）。
...
對照表（欄位、錯誤碼、schema、外部規格）會過期，而模型讀到時無從分辨新舊。所以這類檔案開頭標「取自哪裡、驗證於哪一天」，並寫明過期後該去哪裡重查——本檔的 frontmatter 速查就是這樣做的。
...
另外，徵狀命中不等於該砍。反過來問：這段擋掉的是什麼具體錯誤？答得出來就是低容錯的必要規範，答不出來才是儀式。密度統計（數幾個「必須」）只能拿來挑出要細看的檔案，不能直接當判決。
