...
>
> **這層是前端呼叫 API 的唯一入口**：store / page / component 只 import `app/api/*.api.ts` 的 function，不直接寫 `useHttp` / `useApiFetch` 路徑字串，避免 endpoint 散落各處。
...
- app/composables/useHttp.ts（理解 useHttp().{method} 的 options 形態）
- app/composables/useApiFetch.ts（理解 useApiFetch 的 options 形態）
...
```
useApiFetch (composable)    ← page 層用，SSR-friendly，reactive url
useHttp().{method}          ← store / event handler 用，imperative，含 auth + 401 處理
apiFetch (utils)            ← 兩者底層，純 baseURL + path 替換，無副作用
```

| 場景 | 用什麼 | 理由 |
|---|---|---|
| **GET 列表 / 詳情**（頁面載入時呼叫） | `useApiFetch` | SSR、reactive url、auto-cancel |
| **POST / PATCH / DELETE**（寫入） | `useHttp().{method}` | 寫入永遠用 `$fetch`，不可用 `useFetch`（會在 SSR 重複觸發） |
| **登入 / 註冊**（POST，但不能因 401 跳轉） | `useHttp().post` + `handleUnauthorized: false` | 統一用 useHttp，但避開 401 → navigateTo('/') 副作用 |
| **Blob 下載 / 二進位** | `useHttp().get` + `responseType: 'blob'` | $fetch 才支援 responseType |

> ⚠️ **嚴禁** 在產出的 `*.api.ts` 中：
> - 直接寫 `$fetch(...)`（必須走 `useHttp` / `useApiFetch` / `apiFetch`）
> - 直接寫 `useFetch(...)`（必須走 `useApiFetch`，否則沒帶 auth token + baseURL）
...
```typescript
import type { ApiUseFetchOptions } from '~/composables/useApiFetch'
import type { TeamListItem } from '~/types/api/teams'
import useApiFetch from '~/composables/useApiFetch'

export function listTeams(options?: ApiUseFetchOptions<TeamListItem[]>) {
  return useApiFetch<TeamListItem[]>('/api/v1/teams', options)
}
```
...
export function listPracticePitches(
  ...
  options?: ApiUseFetchOptions<PracticePitchItem[]>,
) {
  return useApiFetch<PracticePitchItem[]>(
...
export function getPracticeDetail(
  practiceId: MaybeRefOrGetter<string>,
  options?: ApiUseFetchOptions<PracticeDetail>,
) {
  return useApiFetch<PracticeDetail>(
...
> ⚠️ path 字串保留 `{paramName}` 原樣，path 變數透過 `pathParams` 物件傳入（由 `apiFetch` 內部 `replaceApiPathParams` 處理）。**不要寫 `/api/v1/players/${playerId}`** —— 失去 endpoint template 的可追蹤性。
...
export function loginCoach(body: LoginBody) {
  return useHttp().post<CoachLoggedInEvent>('/api/v1/auth/login', {
    body,
    handleUnauthorized: false, // 登入失敗回 401 不應觸發 navigateTo('/')
  })
}
```

> ⚠️ 任何 path 以 `/auth/` 開頭、且不需要 token 就能呼叫的端點（login / register），**都要加 `handleUnauthorized: false`**，避免 useHttp 預設攔截 401 把使用者導回 `/`（雖然 login 通常本來就在 `/`，但會 silent 清掉 store 的舊 token，是隱性副作用）。
...
- [ ] GET 列表 / 詳情 → 使用 `useApiFetch`
- [ ] POST / PATCH / DELETE → 使用 `useHttp().{method}`
- [ ] login / register / 不需 token 的 POST → `useHttp().post` + `handleUnauthorized: false`
...
> Phase 1.5 完成後，所有後續 store / page 應該**只 import `~/api`**，禁止再直接寫 `useHttp` / `useApiFetch` / `apiFetch` / `$fetch`（除非是 `~/api` 本身的內部實作）。
