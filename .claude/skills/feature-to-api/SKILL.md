...
> **前置條件**：`/feature-to-flow` 必須先執行完畢（`.flow.md` 已建立）。
...
| 條件 | 模式 | 說明 |
|------|------|------|
| `docs/route-map.yaml` **不存在** | **全量模式** | 從零建立（首次使用） |
...
- `.ai-prompts/ui/ui-config-pm.yaml`（PM 設定，Phase 0 同步用）
...
- **前置檢查**：確認 `docs/e2e-flows/*.flow.md` 存在，若不存在則提示「請先執行 `/feature-to-flow`」
